...
const Navbar = () => {
  ...
  return (
    <header className="fixed top-0 left-0 z-50 w-full transition-all duration-300">
      <AnnouncementBar />
      <nav className={`bg-white shadow-sm transition-all ${isScrolled ? "shadow-lg" : ""}`}>
        <div className="container mx-auto flex items-center justify-between py-4 ">
          {/* Left Section (Logo & Categories) */}
          <div className="flex items-center gap-2">
            <MobileMenu user={user} />
            <Link href="/" className="shrink-0">
              <Image src={logo} alt="Company Logo" width={120} height={30} className="h-9 w-auto" priority />
            </Link>
          ...
          <div className="flex items-center space-x-5">
            ...
            {user ? (
                ...
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="h-10 w-10 rounded-full p-0">
                    <Avatar className="h-9 w-9 border-2 border-gray-100">
                      <AvatarImage src={user.profile.photo} alt={user.displayName} />
                      <AvatarFallback>
                        {user.displayName?.[0] || <HiOutlineUserCircle className="h-6 w-6" />}
                      </AvatarFallback>
                    </Avatar>
                  </Button>
                </DropdownMenuTrigger>
              ...
              <div className="flex items-center gap-2">
                ...
                <Link
                  href="/register"
                  className="hidden rounded-md bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary-dark lg:block"
                >
                  Register
                </Link>
              </div>
