...
import DashboardCustomersOrderDetailsCart from "../../Customer/CustomersOrderDetailsCart";

const AdminDashboardAllOrdersDetails = () => {
  const params = useParams();
  const { data, isLoading } = useGetSingleOrderQuery(params.id);
  const singleOrder: TMainOrder = data?.data;

  return (
    <div className="mb-10 md:m-6">
      {/* Header Section */}
      <div className="flex items-center justify-between">
        <h2 className="flex items-center text-xl font-semibold text-primary sm:text-2xl">
          <ShoppingCart className="mr-2 text-2xl font-bold text-primary sm:mr-3 sm:text-3xl" />
          Order Details
        </h2>
...
export default AdminDashboardAllOrdersDetails;
